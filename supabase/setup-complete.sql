...
-- It handles all types, tables, indexes, and policies safely
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'media_type') THEN
        CREATE TYPE media_type AS ENUM ('none', 'image', 'video', 'logo');
    END IF;
END $$;

-- Add 'location' to field_type if it doesn't exist
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 
